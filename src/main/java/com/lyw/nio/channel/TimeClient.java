 ...
 * whereby one system can request the current time from another system. * Most
 * Linux, BSD and Solaris systems provide RFC 868 time service * on port 37.
 * This simple program will inter-operate with those. * The National Institute
 * of Standards and Technology (NIST) operates * a public time server at
 * time.nist.gov. ** The RFC 868 protocol specifies a 32 bit unsigned value be
 * sent, * representing the number of seconds since Jan 1, 1900. The Java *
 * epoch begins on Jan 1, 1970 (same as unix) so an adjustment is * made by
 * adding or subtracting 2,208,988,800 as appropriate. To * avoid shifting and
 * masking, a four-byte slice of an * eight-byte buffer is used to send/recieve.
 * But getLong( ) * is done on the full eight bytes to get a long value. ** When
 ...
 * or replies may be lost, which means * this code could block forever. ** @author
